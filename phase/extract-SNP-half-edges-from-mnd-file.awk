...
## USAGE: awk [ -v mapq=1 ] -f extract-SNP-half-edges-from-mnd-file.awk <path-to-psf-file> <path-to-mnd-file>
...
($9<mapq||$12<mapq){next}
{
	...
	# 	half-search for at least one overlapping snp
		...
		while (1)
		{
			...
			{
				no_overlapping_snps=1
				break
			}
		}
		if(!no_overlapping_snps)
		{
	...
	if (snplist[1]==""&&snplist[2]==""){next}

	
	n=split(substr(snplist[1],2),a," ")
	m=split(substr(snplist[2],2),b," ")
	
	# just take the first overlapping SNP
	if(a[1]!=""){$2=a[1]}
	if(b[1]!=""){$6=b[1]}
	print
