...
USAGE="
*****************************
	./generate-linkage-map.sh -d <Juneja/...> <path_to_orig_cprops> <path_to_final_tiled_cprops> <path_to_final_tiled_asm>
*****************************
"
...
case "$data" in
	...
	Naumenko)
		...
		chr=3
	;;
	*) echo "Unknown data. Exiting!" >&2 && exit 1
	;;
esac
...
awk -v chr=${chr} 'FILENAME==ARGV[1]{len[$2]=$3; next}FNR<=3{gsub("-",""); c=0; for(i=1;i<=NF;i++){c+=len[$i]}; print FNR"\t"c}' ${current_cprops} ${current_asm} > ${id}.chrom.tiled.sizes

awk -v chr=${chr} 'BEGIN{FS="\t"; OFS="\t"}FILENAME==ARGV[1]{shift[$1+1]=shift[$1]+$2;next}FILENAME==ARGV[2]{pos[substr($8,2)]=$2+1; next}FNR==1{print $0, "Chr-length scaffold (current study)", "Start position (current study)"; next}{if (!pos[$1]){chr="NA"} else if (pos[$1]<shift[2]) {chr=1} else if (pos[$1]<shift[3]) {chr=2} else if (pos[$1]<shift[4]) {chr=3} else {chr="NA"}; if(chr=="NA"){print $0, "NA", "NA"}else{print $0, chr, pos[$1]-shift[chr]}}' ${id}.chrom.tiled.sizes $data"_vs_"${id}"_2D_annotations.txt" ${datafile} > ${data}_map_vs_${id}.txt
